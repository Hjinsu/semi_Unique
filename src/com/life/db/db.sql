...
select * from MEMBER WHERE MEMBER_EMAIL = 'rlead1030@naver.com'
...
select * from FRIEND WHERE FRIEND_MY_ID = 'kakao@진수'
select * from FRIEND WHERE FRIEND_MY_ID = 'rlead92'

update FRIEND set FRIEND_FRIEND_ID05 = 'rlead92'
...
CREATE TABLE ACCOUNT(
	...
	ACCOUNT_DAYS VARCHAR2(10) NOT NULL,
...
select * from HEALTH order by HEALTH_REGDATE DESC
...
INSERT INTO
		ACCOUNT (ACCOUNT_ID, ACCOUNT_IO_NO, ACCOUNT_DATE, ACCOUNT_DAYS, ACCOUNT_IN_CASH, ACCOUNT_OUT_CASH, ACCOUNT_IO_CONTENT,
		ACCOUNT_TOTALCASH)
		SELECT 'kakao@진수', 98, SYSDATE, SELECT TO_CHAR(SYSDATE, 'dy') FROM DUAL,
		1000, 0, 'asdf',
		0
		FROM MEMBER
		WHERE EXISTS( SELECT MEMBER_ID FROM MEMBER WHERE MEMBER_ID = 'kakao@진수')
		
select *from ACCOUNT order by ACCOUNT_IO_NO
